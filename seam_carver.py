...
def compute_energy(image: np.ndarray):
    ...
    H, W, num_colors_channels = image.shape
    # Helper functions
    def calc_energy(row: int, col: int): # assumes image[row][col] is not on the border
        # print(f'{row=} and {col=}')
        x1 = image[row][col - 1] 
        x2 = image[row][col + 1] 
        y1 = image[row - 1][col]
        y2 = image[row + 1][col]
        delta_x = ((x2[0] - x1[0]) ** 2) + ((x2[1] - x1[1]) ** 2) + ((x2[2] - x1[2]) ** 2)
        delta_y = ((y2[0] - y1[0]) ** 2) + ((y2[1] - y1[1]) ** 2) + ((y2[2] - y1[2]) ** 2)
        return (delta_x + delta_y) ** (1 / 2)

    # Populating energy array 
    energy = [] 
    for row in range(H):
        energy.append([])
        for col in range(W):
            if row == 0 or col == 0 or row == H-1 or col == W-1:
                energy[row].append(1000)
            else:
                energy[row].append(calc_energy(row, col))

    # Delete this line after you implemented your algorithm!
    assert((H,W) == np.asarray(energy).shape)
